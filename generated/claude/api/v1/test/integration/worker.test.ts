...
function authedRequest(path: string) {
  return new Request(`https://example.com${path}`, { headers: AUTH_HEADERS });
}

describe("worker integration", () => {
  it("responds to GET /api/v1/health inside the real Workers runtime", async () => {
    const response = await exports.default.fetch(authedRequest("/api/v1/health"));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ message: "Hello World" });
  });

  it("loads exchange rate data through the wrangler.jsonc CSV binding", async () => {
    const response = await exports.default.fetch(authedRequest("/api/v1/currencies"));
    expect(response.status).toBe(200);
  ...
  it("converts currencies end-to-end through the deployed worker", async () => {
    const response = await exports.default.fetch(
      authedRequest("/api/v1/convert?from=USD&to=EUR&amount=100")
    );
    expect(response.status).toBe(200);
    const body = await response.json();
  ...
  it("returns a 400 for an unsupported (non-USD pair) conversion", async () => {
    const response = await exports.default.fetch(
      authedRequest("/api/v1/convert?from=CAD&to=EUR&amount=10")
    );
  ...
  it("returns a 401 for /api/v1/* when no token is provided", async () => {
    const response = await exports.default.fetch("https://example.com/api/v1/health");
    expect(response.status).toBe(401);
  });

  it("returns a 401 for /api/v1/* when the token is wrong", async () => {
    const response = await exports.default.fetch(
